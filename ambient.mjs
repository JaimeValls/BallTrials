// BallTrials · web oficial · FONDO VIVO (modo ambiente)
// Reusa el chassis REAL del juego (gfx.mjs: bola gominola glossy + cara kawaii por sprite;
// emotions.mjs: parpadeo + reacciones) para que las bolas de la web sean IDÉNTICAS a las
// de los vídeos. Sin ganador, sin HUD: bolas que caen entre trampas suaves y se reciclan.
//
// Decisiones del dueño (docs/38 §9): discreto siempre · bolas canónicas · clic vacío = lanzar
// bola + clic cerca = empujón · trampas amables (pegs, trampolín, péndulo) · las bolas REBOTAN
// en las tarjetas del contenido ([data-solid]) · móvil: menos bolas y sin colliders de tarjetas.

import { THREE, makeBall, createParticles, sat } from './chassis/gfx.mjs';
import { EmoBook } from './chassis/emotions.mjs';
import { sfx } from './audio.mjs';

// ── presupuesto / configuración ───────────────────────────────────────────────
const MOBILE = matchMedia('(max-width: 760px)').matches;
const REDUCED = matchMedia('(prefers-reduced-motion: reduce)').matches;
const CAP = MOBILE ? 7 : 14;          // población máxima
const WORLD_H = 12;                    // alto del mundo en unidades (el ancho sigue al aspect)
...
function init(){
  ...
  function resize(){
    ...
    W = WORLD_H * innerWidth / innerHeight;
  ...
  const toWX = px => (px / innerWidth - 0.5) * W;
  const toWY = px => (0.5 - px / innerHeight) * WORLD_H;
  ...
  let rectsDirty = true;
  const solids = MOBILE ? [] : [...document.querySelectorAll('[data-solid]')];
  function refreshRects(){
    rects = [];
    for (const el of solids){
      const r = el.getBoundingClientRect();
      if (r.bottom < -40 || r.top > innerHeight + 40 || r.width === 0) continue;   // fuera de pantalla
  ...
  addEventListener('resize', () => { resize(); rectsDirty = true; });
