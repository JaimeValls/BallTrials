...
function drawEye(x, u, ecx, ecy, side, spec){
  ...
  if (shape === 'round' || shape === 'wide' || shape === 'narrow'){
    ...
    ell(x, ecx, ecy, rx, ry, WHITE, INK, u(5.0));                 // contorno un pelín más fino: el ojo "respira"
    const pr = u(PUPR * pupm), ox = u(gx * 9), oy = u(gy * 9);    // gaze 11→9: pupila gigante no debe salirse del blanco
...
function drawFace(spec){
  ...
  drawEye(x, u, lx, ey, 'L', spec); drawEye(x, u, rx, ey, 'R', spec);
